...
`profiles/nikechan-another-world/` は、同じ `nikechan-hermes` Hermes gateway配下で扱う別人格・別surface用profile資材です。xangiには接続しません。
...
1. `.env.example` を参考に、VPS上で `/opt/nikechan-hermes/.env` を作成する
...
node scripts/nikechan-hermes.mjs propose --source-mode presence --candidates-json '[{"text":"...","reason":"..."}]'
...
node scripts/nikechan-hermes.mjs approve --ids 1
node scripts/nikechan-hermes.mjs cancel --reason "見送り"
node scripts/nikechan-hermes.mjs mention-context
node scripts/nikechan-hermes.mjs mention-propose --items-json '{"items":[...]}'
...
node scripts/nikechan-hermes.mjs hashtag-execute --items-json '{"items":[...]}'
node scripts/nikechan-hermes.mjs doctor
node scripts/nikechan-hermes.mjs preflight-live
node scripts/nikechan-hermes.mjs release-mode --set dry-run
```
...
```bash
node scripts/nikechan-hermes.mjs preflight-live
node scripts/nikechan-hermes.mjs release-mode --set live --confirm LIVE_X_POSTING
```
...
VPSではHermes gateway内蔵cronが `/opt/nikechan-hermes/cron/jobs.json` を読みます。self-tweet / mention-reaction / hashtag-reaction は `no_agent: false` のHermes agent jobとして動き、profile内skillを読み込んでCLIのguard/pending/post境界を呼びます。
...
```bash
systemctl status ai.hermes.gateway-nikechan-hermes --no-pager
docker compose logs --tail=100 nikechan-hermes
```
